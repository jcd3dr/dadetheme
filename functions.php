...
function dadecore_theme_add_page_options_meta_box() {
    add_meta_box(
        'dadecore_theme_page_options',      // ID único del meta box.
        'Opciones de Página',        // Título del meta box que verá el usuario.
        'dadecore_theme_page_options_html', // Función que muestra el contenido del meta box.
        'page',                      // Mostrar solo en 'page' (páginas), no en 'post' (entradas).
        'side',                      // Lugar donde aparecerá ('side' o 'normal').
        'default'                    // Prioridad.
    );
}
add_action( 'add_meta_boxes', 'dadecore_theme_add_page_options_meta_box' );
...
function dadecore_theme_page_options_html( $post ) {
    ...
    wp_nonce_field( 'dadecore_theme_save_page_options', 'dadecore_theme_page_options_nonce' );
    ?>
    <p>
        <input type="checkbox" id="dadecore_theme_hide_title_field" name="dadecore_theme_hide_title_field" value="1" <?php checked( $value, '1' ); ?>>
        <label for="dadecore_theme_hide_title_field">Ocultar Título de la Página</label>
    </p>
    <?php
}
...
function dadecore_theme_save_page_options( $post_id ) {
    ...
    if ( ! isset( $_POST['dadecore_theme_page_options_nonce'] ) || ! wp_verify_nonce( $_POST['dadecore_theme_page_options_nonce'], 'dadecore_theme_save_page_options' ) ) {
        return;
    }

    // Guardamos o borramos el dato según si la casilla está marcada.
    if ( isset( $_POST['dadecore_theme_hide_title_field'] ) && '1' === $_POST['dadecore_theme_hide_title_field'] ) {
        update_post_meta( $post_id, '_hide_page_title_key', '1' );
    } else {
        delete_post_meta( $post_id, '_hide_page_title_key' );
    }
}
add_action( 'save_post_page', 'dadecore_theme_save_page_options' );
